...
func _ready():
	...
	SteamLobby.connect("lobby_created", on_lobby_created)
	SteamLobby.connect("lobby_joined",on_lobby_joined)
	SteamLobby.connect("lobby_owner_changed",on_lobby_owner_changed)
	SteamLobby.connect("player_joined_lobby", on_player_joined)
	SteamLobby.connect("player_left_lobby",on_player_left)
	SteamLobby.connect("chat_message_received", on_chat_message_received)
	...
	SteamNetwork.connect("peer_status_updated",on_peer_status_changed)
	SteamNetwork.connect("all_peers_connected", on_all_peers_connected)
	
	
	create_lobby_btn.connect("pressed",on_create_lobby_pressed)
	invite_friend_btn.connect("pressed", on_invite_friend_pressed)
	
	chat_line_edit.connect("text_entered", on_chat_text_entered)
	chat_send_btn.connect("pressed", on_chat_send_pressed)

	rpc_on_server_btn.connect("pressed", on_rpc_server_pressed)
	
	change_owner_btn.connect("pressed",on_change_owner_pressed)
